...
{ 
    ...
    // IO io_obj( 31 , 5  , {1000, 1000, 1000, 1000, 1000}); // num_points,  num_k  , max_dim_vec  //  31 , 5  , {1000, 1000, 1000, 1000, 1000} // std::stoi(argv[1]) , std::stoi(argv[2]) , temp_vec

    // io_obj.write_dataset ();

    // io_obj.ReadFile();

    KDTree tree_obj(vec_2d);//making kdtree object//I should use coord_data instead of vec_2d everywhere

    tree_obj.populate_tree(vec_2d);//calling populate_tree method onthe tree object

    std::vector<std::vector<int>>inorder_vec;
    
    inorder_vec = tree_obj.inOrder(tree_obj.root);//

    for (int i = 0 ; i < inorder_vec.size() ; i++){

        for(int j = 0 ; j < inorder_vec[i].size() ; j++){

            std::cout<<inorder_vec[i][j]<< " ";
        }
